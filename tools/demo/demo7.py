...
def parse_args_to_cfg():
    ...
    parser.add_argument("-s", "--static_cam", action="store_true", help="如果指定，则跳过 DPVO 模块")
    parser.add_argument("--verbose", action="store_true", help="如果指定，则显示详细的中间结果")
    parser.add_argument("--person", type=int, default=0, help="指定处理的目标编号")
    ...
    # Output
    Log.info(f"[输出地址]: {cfg.output_dir}")
    Path(cfg.output_dir).mkdir(parents=True, exist_ok=True)
    Path(cfg.preprocess_dir).mkdir(parents=True, exist_ok=True)

    # Copy raw-input-video to video_path
    Log.info(f"[拷贝视频] {video_path} -> {cfg.video_path}")
...
def run_preprocess(cfg):
    ...
    tracker = Tracker()
    bbx_xyxy = tracker.get_one_track(video_path).float() # (L, 4) # (L, 4)
...
def render_incam(cfg):
    incam_video_path = Path(cfg.paths.incam_video+'_person-'+str(person)+".mp4")
...
if __name__ == "__main__":
    ...
    #os codigos abaixo nao funcionaram
    # pred_np['model_output']['pred_x'] = pred['net_outputs']['model_output']['pre_x'].cpu().numpy()
    # pred_np['model_output']['pred_cam'] = pred['net_outputs']['model_output']['pre_cam'].cpu().numpy()
    # pred_np['model_output']['pred_context'] = pred['net_outputs']['model_output']['pre_context'].cpu().numpy()
    import pickle
